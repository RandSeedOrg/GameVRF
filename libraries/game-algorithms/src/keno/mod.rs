use crate::common::{chacha::ChaCha20RNGGenerator, seed_mixer::SeedMixableNumber};
...
pub fn generate_numbers<T: SeedMixableNumber>(seed: [u8; 32], round: T, count: usize, min: u8, max: u8) -> Vec<u8> {
  ...
  // 步骤 1: 将动态因子通过哈希或异或混入原始种子，从而保障当前轮次抽签结果的独立性和唯一性。
  ...
  let random_numbers = u8::chacha20_rand(mixed_seed, min, max, count);
